...
-- Test objects
CREATE TABLE public.rv_base (id int, name text, email text, active boolean);

CREATE VIEW public.rv_target AS
    SELECT id, name, email FROM public.rv_base WHERE active;

CREATE VIEW public.rv_dep_l1 AS
    SELECT id, name FROM public.rv_target;

CREATE VIEW public.rv_dep_l2 AS
    SELECT id, name FROM public.rv_dep_l1;

CREATE MATERIALIZED VIEW public.rv_mat_dep AS
    SELECT id, name FROM public.rv_target;

GRANT SELECT ON public.rv_target TO PUBLIC;
GRANT SELECT ON public.rv_dep_l1 TO PUBLIC;
